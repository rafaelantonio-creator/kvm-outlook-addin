...
function aplicarTemplate() {
  ...
  // Verifica se o corpo já tem conteúdo antes de sobrescrever
  item.body.getAsync(Office.CoercionType.Html, function(result) {
    if (result.status === Office.AsyncResultStatus.Succeeded) {
      var conteudoAtual = result.value.replace(/<[^>]*>/g, '').trim();
 
      // Só insere se o corpo estiver vazio
      if (conteudoAtual.length === 0) {
        inserirTemplate();
      }
    } else {
      // Se não conseguir verificar, insere mesmo assim
      inserirTemplate();
    }
  });
}
...
function inserirTemplate() {
  ...
  Office.context.mailbox.item.body.setAsync(
    ...
    function(result) {
      if (result.status !== Office.AsyncResultStatus.Succeeded) {
        console.error('Erro ao inserir template: ' + result.error.message);
      }
    }
  );
}
